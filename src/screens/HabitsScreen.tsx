        ...
        >
          ...
          >
            ...
            <Text style={[styles.modalButtonText, { color: colors.text }]}>
              Сортировать привычки
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={onSortCategories}
            style={[
              styles.modalButton,
              { backgroundColor: colors.inputBackground },
            ]}
...
export default function HabitsScreen() {
  ...
  const [selectedHabitForAction, setSelectedHabitForAction] =
    useState<Habit | null>(null);
  const [selectedHabitForDetail, setSelectedHabitForDetail] =
    useState<Habit | null>(null);
  ...
  const handleDeleteCategory = useCallback(
    async (categoryId: string, categoryName: string) => {
      Alert.alert(
        ...
        "Привычки в этой категории останутся без категории.",
        [
  ...
  const onRefresh = useCallback(async () => {
    ...
    if (user?.id) {
      await Promise.all([fetchHabits(user.id), fetchCategories(user.id)]);
    }
    setIsRefreshing(false);
  }, [user, fetchHabits, fetchCategories]);

  const handleUpdateProgress = useCallback(
    async (habitId: string, newProgress: number) => {
      await updateHabitProgress(habitId, newProgress, currentDate);
    },
    [updateHabitProgress, currentDate],
  );

  const filteredHabits = useMemo(() => {
    if (selectedCategoryId === "All" || !selectedCategoryId) return habits;
    if (selectedCategoryId === "Uncategorized")
      return habits.filter((h) => !h.categories || h.categories.length === 0);
    return habits.filter((h) =>
      h.categories.some((c) => c.id === selectedCategoryId),
    );
  ...
  const handleArchiveHabit = useCallback(
    async (habitId: string) => {
      Alert.alert(
        ...
        "Ее можно будет восстановить в настройках.",
        [
          { text: "Отмена", style: "cancel" },
          {
            text: "Архивировать",
  ...
  const handleHabitPress = (habit: Habit) => {
    console.log("НАЖАТИЯ выбрана привычка ->", habit.name);
  ...
  console.log(
    "РЕНДЕР HabitsScreen: Состояние selectedHabitForDetail ->",
    selectedHabitForDetail ? selectedHabitForDetail.name : null,
  );
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.header}>
        ...
        >
          <TouchableOpacity
            onPress={() => navigation.navigate({ name: "Journal", params: {} })}
          ...
          >
            ...
            <TouchableOpacity
              onPress={() =>
                ...
                handleEditHabit(selectedHabitForAction)
              }
            ...
            <TouchableOpacity
              onPress={() =>
                selectedHabitForAction &&
                handleArchiveHabit(selectedHabitForAction.id)
              }
