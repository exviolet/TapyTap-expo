...
import { format, subDays, startOfDay, isWithinInterval, parseISO, differenceInCalendarDays, formatDistanceToNow } from 'date-fns';
...
const HabitDetailSheet: React.FC<Props> = ({ habit, onClose }) => {
    ...
    const { allCompletions, streaks, archiveHabit, notes, fetchNotesForHabit, addOrUpdateNote, fetchAllNotes } = useHabitStore();
    ...
    // --- Расчеты KPI и Цели ---
    const { totalCompletions, thirtyDayPercentage, goalSeriesText, bestStreak } = useMemo(() => {
        if (!habit) return { totalCompletions: 0, thirtyDayPercentage: 0, goalSeriesText: '', bestStreak: 0 };
        ...
        if (habitCompletions.length > 0) {
            ...
            for (let i = 0; i < habitCompletions.length - 1; i++) {
                const diff = differenceInCalendarDays(habitCompletions[i], habitCompletions[i + 1]);
                if (diff === 1) {
                    currentStreak++;
                } else {
                    ...
                    {habit && (
                        <>
                            ...
                            <View style={styles.tabContainer}>
                                <TouchableOpacity style={[styles.tabButton, activeTab === 'overview' && { backgroundColor: colors.accent }]} onPress={() => setActiveTab('overview')}>
                                    <LucideIcons.LayoutGrid size={16} color={activeTab === 'overview' ? '#FFF' : colors.text} />
                                    <Text style={[styles.tabText, { color: activeTab === 'overview' ? '#FFF' : colors.text }]}>Обзор</Text>
                                </TouchableOpacity>
                                <TouchableOpacity style={[styles.tabButton, activeTab === 'journal' && { backgroundColor: colors.accent }]} onPress={() => setActiveTab('journal')}>
                                    <LucideIcons.BookText size={16} color={activeTab === 'journal' ? '#FFF' : colors.text} />
                                    <Text style={[styles.tabText, { color: activeTab === 'journal' ? '#FFF' : colors.text }]}>Журнал</Text>
                                </TouchableOpacity>
                            </View>
                            {activeTab === 'overview' ? (
                                <>
                                    <View style={styles.kpiContainer}>
                                        ...
                                        <View style={styles.kpiBox}>
                                            ...
                                            <Text style={[styles.kpiValue, { color: colors.text }]}>{bestStreak}</Text>
                                            <Text style={[styles.kpiLabel, { color: colors.textSecondary }]}>Лучшая серия</Text>
                                        </View>
                                        <View style={styles.kpiBox}>
                                ...
                                <View style={styles.journalContainer}>
                                    <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>ЗАМЕТКА НА СЕГОДНЯ</Text>
                                    <View style={[styles.inputContainer, { backgroundColor: colors.inputBackground }]}>
                                        ...
                                        />
                                        <TouchableOpacity
                                            style={[styles.addNoteButton, { backgroundColor: colors.accent, opacity: isPostingNote ? 0.6 : 1 }]}
                                            onPress={handleAddNote}
                                            disabled={isPostingNote}
                                        >
                                            {isPostingNote ? <ActivityIndicator size="small" color="#FFF" /> : <LucideIcons.Send size={20} color="#FFF" />}
                                        </TouchableOpacity>
                                    </View>
                                    {isJournalLoading ? (
                                        ...
                                        <>
                                            {notes.length > 0 ? (
                                                notes.map(note => (
                                                    <View key={note.id} style={[styles.noteCard, { backgroundColor: colors.inputBackground }]}>
                                                        <Text style={[styles.noteDate, { color: colors.accent }]}>
                                                            {formatDistanceToNow(parseISO(note.note_date), { addSuffix: true, locale: ru })}
                                                        </Text>
                                                        <Text style={[styles.noteContent, { color: colors.text }]}>{note.content}</Text>
                                                    </View>
                                                ))
                                            ) : (
                                                <Text style={{ color: colors.textSecondary, textAlign: 'center', marginTop: 20, fontStyle: 'italic' }}>
                                                    Заметок пока нет
                                                </Text>
                                            )}
                                            <TouchableOpacity style={[styles.viewAllButton, { backgroundColor: colors.accent }]} onPress={handleViewAllNotes}>
                                                <Text style={[styles.viewAllText, { color: '#FFF' }]}>Посмотреть все</Text>
                                            </TouchableOpacity>
                                        </>
                                    )}
                                </View>
...
const styles = StyleSheet.create({
    ...
    tabContainer: { flexDirection: 'row', backgroundColor: '#7676801F', borderRadius: 12, padding: 4, marginBottom: 20 },
    ...
    noteInput: { flex: 1, fontSize: 15, minHeight: 40, maxHeight: 120, paddingTop: 10 },
    addNoteButton: { width: 40, height: 40, borderRadius: 10, justifyContent: 'center', alignItems: 'center', alignSelf: 'flex-end', marginLeft: 8 },
